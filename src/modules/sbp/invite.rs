...
pub async fn invite(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let buttons = vec![serenity::CreateActionRow::Buttons(vec![
        serenity::CreateButton::new(&ctx.id().to_string()).label("Принять приглашение"),
    ])];
    ...
    handle_button(
        ...
        move |press| async move {
            ...
            sqlx::query("INSERT INTO sbp_users (id) VALUES ($1)")
                .bind::<i64>(press.user.id.into())
                .execute(&mut *tx)
                .await?;

            sqlx::query("INSERT INTO sbp_invites (user_id, invited_user_id) VALUES ($1, $2)")
        ...
        move || async move {
            ...
            msg.edit(ctx, CreateReply::default().components(buttons).embed(embed))
                .await?;
            return Ok(());
        },
    )
