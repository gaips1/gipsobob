...
pub async fn my_harem(ctx: Context<'_>,) -> Result<(), Error> {
    ...
    let harem_author = if
        harem_author_id != ctx.author().id.get() {
    ...
    let embed = serenity::CreateEmbed::new()
        ...
        .description(format!("Участники гарема (последние 30): {}\nГарем был создан <t:{}:R>", harem_users_text, harem_users.first().unwrap().2.timestamp()))
        .colour(serenity::colours::branding::BLURPLE);

    let buttons = vec![serenity::CreateActionRow::Buttons(vec![
        serenity::CreateButton::new("harem:leave").label("Покинуть гарем").style(serenity::ButtonStyle::Danger),
    ])];
...
pub async fn handle_harem_leave_button(
    ...
    match custom_id {
        "harem:leave" => {
            let buttons = vec![serenity::CreateActionRow::Buttons(vec![
                serenity::CreateButton::new("harem:leave:yes").label("Да").style(serenity::ButtonStyle::Danger),
            ])];
        ...
        "harem:leave:yes" => {
            ...
            if user_harem.0 as u64 == press.user.id.get() {
                ...
                sqlx::query(
                    "UPDATE users SET harem_id = null WHERE id = $1"
                )
                    .bind(press.user.id.get() as i64)
                    .execute(&data.pool)
                    .await?;
                ...
                let harem_author = serenity::UserId::new(user_harem.0 as u64).to_user(ctx).await;
                let Ok(harem_author) = harem_author else { return Ok(()) };
                let _ = harem_author.dm(ctx, serenity::CreateMessage::new().content(format!("{} покинул ваш гарем", press.user.mention()))).await;
            }
        }
        _ => {}
    }
