...
pub async fn invite_to_harem(ctx: Context<'_>,) -> Result<(), Error> {
    ...
    let author_harem_id: Option<i64> = sqlx::query_scalar(
        "SELECT id FROM harems WHERE author_id = $1"
    )
    ...
    let Some(author_harem_id) = author_harem_id else {
        ctx.say("Вы в данный момент не находитесь в гареме или не являетесь его создателем").await?;
        return Ok(());
    };
    ...
    let msg = ctx.send(poise::CreateReply::default().embed(embed.clone()).components(buttons).ephemeral(false)).await?;

    handle_button(ctx, &btn_id, 3600, 
        move |press| {
            async move {
                let user_harem_id: Option<Option<i64>> = sqlx::query_scalar(
        ...
        move || {
            async move {
                let buttons = vec![serenity::CreateActionRow::Buttons(vec![
                    serenity::CreateButton::new("meow").label("Приглашение истекло").disabled(true)
                ])];

                let _ = msg.edit(ctx, poise::CreateReply::default().embed(embed).components(buttons)).await;
