...
pub async fn global_check(ctx: Context<'_>) -> Result<bool, Error> {
    ...
    let user_is_banned = sqlx::query_scalar("SELECT is_banned FROM users WHERE id = $1")
        .bind::<i64>(user_id.into())
        .fetch_optional(pool)
        .await;
    ...
    match user_is_banned {
        ...
        Ok(None) => {
            let _ = sqlx::query("INSERT INTO users (id) VALUES ($1)")
                .bind::<i64>(user_id.into())
                .execute(pool)
                .await;
            Ok(true)
...
pub async fn sbp_check(ctx: Context<'_>) -> Result<bool, Error> {
    ...
    let is_sbp_user_exists: Result<bool, sqlx::Error> =
